...
import json
import os
from models.signals.alpha_calculations import AlphaCalculations
...
class AlphaFactors:
    """
    Alpha factor calculation and evaluation.
    Works with alpha_calculations.py for implementing specific factor logic.
    """

    def __init__(self, config_path: Optional[str] = None):
        self.config = self._load_config(config_path)
        self.factor_performance = {}
        self.calculations = AlphaCalculations()
    ...
    def calculate_alpha_factors(self,
        ...
        if 'vwap' not in df_result.columns:
            df_result['vwap'] = (df_result['volume'] * (
                        df_result['high'] + df_result['low'] + df_result['close']) / 3).cumsum() / df_result[
                                    'volume'].cumsum()
        ...
        for factor_name in factors_to_calculate:
            # Skip if factor already calculated
            if factor_name in df_result.columns:
                continue

            try:
                # Get the calculation method
                calculation_method = getattr(self.calculations, factor_name, None)

                if calculation_method:
                    # Call the factor calculation function
                    df_result[factor_name] = calculation_method(df_result)
                    logger.debug(f"Calculated {factor_name}")
                else:
                    logger.warning(f"Factor calculation method not found: {factor_name}")
    ...
    def evaluate_factor_performance(self,
        ...
        for factor in factor_names:
            if factor in data.columns:
                ...
                data[f'{factor}_quintile'] = pd.qcut(data[factor].rank(method='first'), 5, labels=False,
                                                     duplicates='drop')
