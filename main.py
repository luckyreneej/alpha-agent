...
class AlphaAgentSystem:
    ...
    def run_backtest(self, backtest_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            backtester = BacktestEngine(
                initial_capital=backtest_config.get('initial_capital', self.config['trading'].get('initial_capital', 100000)),
    ...
    def _get_strategy_function(self, strategy_name: Optional[str] = None):
        ...
        def alpha_factor_strategy(historical_data, current_day, **params):
            ...
            relevant_history = historical_data.iloc[-lookback:].copy() if len(historical_data) > lookback else historical_data.copy()
                    ...
                    if not ticker_data.empty:
                        ...
                        if last_row is not None:
                            ...
                            if alpha_cols:
                                ...
                                normalized = [(val - np.mean(alpha_values)) / np.std(alpha_values) if np.std(alpha_values) > 0 else 0 
